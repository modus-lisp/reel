...
| **H.264** | *Decode only; Baseline and most of Main.* Constrained Baseline, CAVLC, 4:2:0, 8-bit, progressive: NAL and RBSP handling, Exp-Golomb, SPS/PPS/slice headers, CAVLC residuals, the integer 4x4 transform and both Hadamard DC transforms, all nine Intra4x4 modes plus Intra16x16 and chroma, the in-loop deblocking filter, and inter prediction: P slices with quarter-pel motion compensation, multiple references and every partition size. **Bit-exact with ffmpeg's filtered output** on every frame of eleven fixtures and on an MP4's video track, at about 118 fps on all-intra 640x360 and 71 at 1080p, decoding independent pictures concurrently. CABAC, weighted prediction, reference list reordering and B slices with both direct modes all work, so **Main profile decodes** — an ordinary YouTube file plays end to end, bit-exact. High profile, 4:2:2 and more than 8 bits are refused loudly rather than decoded wrong. |
