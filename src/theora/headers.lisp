...
(defun parse-setup (bytes i)
  ...
  (let ((br (make-br bytes :start 7 :end (length bytes))))
    ...
    (let* ((matrices (1+ (read-bits br 9)))
      ...
      (let ((qr-count (make-array '(2 3) :element-type '(signed-byte 32) :initial-element 0))
            (qr-size (make-array '(2 3 64) :element-type '(signed-byte 32) :initial-element 0))
            (qr-base (make-array '(2 3 65) :element-type '(signed-byte 32) :initial-element 0)))
        (dotimes (inter 2)
          (dotimes (plane 3)
            (let ((newqr 1))
