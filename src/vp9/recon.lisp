...
(defun %inter-recon (st)
  ...
  (let* ((f (st-frame st)) (bs (st-bs st))
    ...
    (declare (type fixnum bs row col filter ys cs ybase cbase))
    (dotimes (lx (if (st-bcomp st) 2 1))
      (let* ((ref (aref (st-ref-frames st) (aref (st-bref st) lx)))
             ...
             (uvx 0) (uvy 0))
        (declare (type fixnum rys rcs w h cw ch uvx uvy))
        (flet ((luma (dx dy bw bh which)
                 (declare (type fixnum dx dy bw bh which))
