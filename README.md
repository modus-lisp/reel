...
**VP8, in pure Common Lisp — both directions.** An encoder built for real-time desktop
streaming, and a decoder that is bit-exact with libvpx. No FFI, and no dependencies at
all: the codec is arithmetic over octet vectors, and a dependency here would be one for
every image, video and transport user above it.
...
decoder against libvpx over eight clips, and `cassette/inspect/test-encode-mux.lisp` for
ffmpeg decoding what this encoder produced.
