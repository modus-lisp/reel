...
(defun %pred-intra (mode dst stride base sz l a)
  ...
  (macrolet ((tp (k) `(aref a (+ 32 ,k)))
    ...
    (case mode
      ...
      (#.+pred-diag-dl+
       (if (= sz 4)
           ...
           (progn
             ...
             (put 3 3 (tp 7)))
           (let ((v (make-array 32 :element-type '(signed-byte 32))))
      ...
      (#.+pred-diag-dr+
       (let ((v (make-array 64 :element-type '(signed-byte 32))))
         (declare (dynamic-extent v))
         (dotimes (i (- sz 2))
      ...
      (#.+pred-vert-right+
       (let ((ve (make-array 48 :element-type '(signed-byte 32)))
             (vo (make-array 48 :element-type '(signed-byte 32)))
             (half (ash sz -1)))
      ...
      (#.+pred-hor-down+
       (let ((v (make-array 96 :element-type '(signed-byte 32))))
         (declare (dynamic-extent v))
         (dotimes (i (- sz 2))
      ...
      (#.+pred-vert-left+
       (if (= sz 4)
           ...
           (let ((ve (make-array 32 :element-type '(signed-byte 32)))
                 (vo (make-array 32 :element-type '(signed-byte 32))))
