    ...
    (let* ((cw4 (ash w4 -1)) (cex (ash end-x -1)) (ceh (ash end-y -1))
      ...
      (dotimes (pl 2)
        (let ((dst (the octets (aref (fr-planes f) (1+ pl))))
              ...
              (coeffs (the coefs (aref (st-uvcoeffs st) pl)))
...
(defun %inter-recon (st)
  "Predict one inter block from its reference or references, then add its residual.

   A block of 8x8 or larger predicts once with one vector.  A smaller one predicts its luma in two
   or four pieces with a vector each — and its CHROMA in one piece, with those vectors averaged,
   because at half the resolution the pieces would be two samples wide and an eight-tap filter needs
   more than that to work on."
  (declare (type state st) (optimize (speed 3) (safety 1)))
  (let* ((f (st-frame st)) (bs (st-bs st))
         (row (st-row st)) (col (st-col st))
         (mv (st-bmv st))
         (filter (st-bfilter st))
         (ys (aref (fr-stride f) 0)) (cs (aref (fr-stride f) 1))
         (ybase (+ (* 8 row ys) (* 8 col)))
         (cbase (+ (* 4 row cs) (* 4 col)))
         (tmp (st-mc-tmp st)) (win (st-mc-win st))
         (dy0 (the octets (aref (fr-planes f) 0)))
         (du0 (the octets (aref (fr-planes f) 1)))
         (dv0 (the octets (aref (fr-planes f) 2))))
    (declare (type fixnum bs filter ybase cbase) (type dim row col ys cs))
    (dotimes (lx (if (st-bcomp st) 2 1))
      (let* ((ref (aref (st-ref-frames st) (aref (st-bref st) lx)))
             (avg (plusp lx))
             (yp (the octets (aref (fr-planes ref) 0)))
             (up (the octets (aref (fr-planes ref) 1)))
             (vp (the octets (aref (fr-planes ref) 2)))
             (rys (aref (fr-stride ref) 0)) (rcs (aref (fr-stride ref) 1))
             (w (fr-width ref)) (h (fr-height ref))
             (cw (ash (1+ w) -1)) (ch (ash (1+ h) -1))
             (uvx 0) (uvy 0))
        (declare (type fixnum uvx uvy) (type dim rys rcs w h cw ch))
        (flet ((luma (dx dy bw bh which)
                 (declare (type (integer 0 64) dx dy bw bh) (type fixnum which))
                 (%mc-block dy0 ys (+ ybase (* dy ys) dx) yp rys w h
                            (+ (* 8 col) dx) (+ (* 8 row) dy)
                            (aref mv which lx 0) (aref mv which lx 1) bw bh filter avg t
                            tmp win))
               (chroma (bw bh)
                 (declare (type fixnum bw bh))
                 (%mc-block du0 cs cbase up rcs cw ch (* 4 col) (* 4 row)
                            uvx uvy bw bh filter avg nil tmp win)
                 (%mc-block dv0 cs cbase vp rcs cw ch (* 4 col) (* 4 row)
                            uvx uvy bw bh filter avg nil tmp win)))
          (macrolet ((avg2 (a b) `(%rounded-div (+ ,a ,b) 2))
                     (avg4 (a b c d) `(%rounded-div (+ ,a ,b ,c ,d) 4)))
            (cond
              ((= bs +bs-8x4+)
               (luma 0 0 8 4 0) (luma 0 4 8 4 2)
               (setf uvx (avg2 (aref mv 0 lx 0) (aref mv 2 lx 0))
                     uvy (avg2 (aref mv 0 lx 1) (aref mv 2 lx 1)))
               (chroma 4 4))
              ((= bs +bs-4x8+)
               (luma 0 0 4 8 0) (luma 4 0 4 8 1)
               (setf uvx (avg2 (aref mv 0 lx 0) (aref mv 1 lx 0))
                     uvy (avg2 (aref mv 0 lx 1) (aref mv 1 lx 1)))
               (chroma 4 4))
              ((> bs +bs-8x8+)                  ; four 4x4 partitions
               (luma 0 0 4 4 0) (luma 4 0 4 4 1) (luma 0 4 4 4 2) (luma 4 4 4 4 3)
               (setf uvx (avg4 (aref mv 0 lx 0) (aref mv 1 lx 0)
                               (aref mv 2 lx 0) (aref mv 3 lx 0))
                     uvy (avg4 (aref mv 0 lx 1) (aref mv 1 lx 1)
                               (aref mv 2 lx 1) (aref mv 3 lx 1)))
               (chroma 4 4))
              (t
               (let ((bw (* 4 (aref +block-size-wh+ 0 bs 0)))
                     (bh (* 4 (aref +block-size-wh+ 0 bs 1))))
                 (declare (type fixnum bw bh))
                 (luma 0 0 bw bh 0)
                 (setf uvx (aref mv 0 lx 0) uvy (aref mv 0 lx 1))
                 (chroma (ash bw -1) (ash bh -1)))))))))
    ;; ---- and the residual, which for an inter block is always the plain transform
    (unless (st-skip st)
      (let* ((tx (st-tx st)) (uvtx (st-uvtx st))
             (lossless (h-lossless (st-h st)))
             (w4 (ash (%bw4 bs) 1)) (h4 (ash (%bh4 bs) 1))
             (end-x (min (* 2 (- (st-cols st) col)) w4))
             (end-y (min (* 2 (- (st-rows st) row)) h4)))
        (declare (type fixnum tx uvtx w4 h4 end-x end-y))
        (let ((step (ash 1 tx)) (sz (ash 4 tx)) (n 0))
          (declare (type fixnum step sz n))
          (loop for y of-type fixnum from 0 below end-y by step
                do (loop for x of-type fixnum from 0 below end-x by step
                         do (let ((eob (aref (st-eob st) n)))
                              (declare (type fixnum eob))
                              (when (plusp eob)
                                (%itxfm-add dy0 ys (+ ybase (* 4 y ys) (* 4 x))
                                            (st-coeffs st) sz tx 0 eob lossless (* 16 n))))
                            (incf n (* step step)))))
        (let ((step (ash 1 uvtx)) (sz (ash 4 uvtx))
              (cex (ash end-x -1)) (ceh (ash end-y -1)))
          (declare (type fixnum step sz cex ceh))
          (dotimes (pl 2)
            (let ((dst (if (zerop pl) du0 dv0))
                  (coeffs (the coefs (aref (st-uvcoeffs st) pl)))
                  (eobs (the (simple-array (signed-byte 32) (*)) (aref (st-uveob st) pl)))
                  (n 0))
              ...
              (loop for y of-type fixnum from 0 below ceh by step
                    do (loop for x of-type fixnum from 0 below cex by step
                             do (let ((eob (aref eobs n)))
                                  (declare (type fixnum eob))
                                  (when (plusp eob)
                                    (%itxfm-add dst cs (+ cbase (* 4 y cs) (* 4 x))
                                                coeffs sz uvtx 0 eob lossless (* 16 n))))
                                (incf n (* step step)))))))))
    (values)))
