...
(defun %read-mv-component (st idx hp)
  ...
  (let* ((c (st-c st)) (p (fp-p (st-fp st)))
    ...
    (if (plusp cl)
        (progn
          (dotimes (m cl)
            (setf n (logior n (ash (bool-bit c (aref (pr-mv-bits p) idx m)) m))))
          (setf n (ash n 3))
          (setf n (logior n (ash (bool-tree c +mv-fp-tree+ (pr-mv-fp p) (* 3 idx)) 1)))
          (setf n (logior n (if (plusp hp) (bool-bit c (aref (pr-mv-hp p) idx)) 1)))
          (incf n (ash 8 cl)))
        (progn
          ...
          (setf n (logior n (if (plusp hp) (bool-bit c (aref (pr-mv-class0-hp p) idx)) 1)))))
  ...
  (let ((mv (st-bmv st)) (m (st-mode st)) (fixref (h-fix-comp-ref h)))
          ...
          (setf (st-bcomp st)
                (if (/= (fp-comp-pred-mode (st-fp st)) +pred-switchable+)
                    ...
                    (let ((k (%ref-ctx-comp st h have-a have-l col row7 fixref)))
                      ...
                      (plusp (bool-bit c (aref (pr-comp p) k))))))
          (if (st-bcomp st)
              ...
              (let* ((fix-idx (aref (h-sign-bias h) fixref))
                ...
                (setf (aref (st-bref st) var-idx)
                      (aref (h-var-comp-ref h) (bool-bit c (aref (pr-comp-ref p) k)))))
              (let ((k (%ref-ctx-single0 st h have-a have-l col row7 fixref)))
                (declare (type fixnum k))
                (if (zerop (bool-bit c (aref (pr-single-ref p) k 0)))
                    (setf (aref (st-bref st) 0) 0)
                    (let ((k2 (%ref-ctx-single1 st h have-a have-l col row7 fixref)))
                      (declare (type fixnum k2))
                      (setf (aref (st-bref st) 0)
                            (1+ (bool-bit c (aref (pr-single-ref p) k2 1))))))))))
    ...
    (if (> bs +bs-8x8+)
        (let ((k (aref +inter-mode-ctx-lut+
          ...
          (declare (type fixnum k))
          (setf (aref m 0) (bool-tree c +inter-mode-tree+ (pr-mv-mode p) (* 3 k)))
          (%fill-mv st 0 (aref m 0) 0)
          (if (/= bs +bs-8x4+)
              (progn (setf (aref m 1) (bool-tree c +inter-mode-tree+ (pr-mv-mode p) (* 3 k)))
          ...
          (if (/= bs +bs-4x8+)
              (progn
                (setf (aref m 2) (bool-tree c +inter-mode-tree+ (pr-mv-mode p) (* 3 k)))
                (%fill-mv st 2 (aref m 2) 2)
                (if (/= bs +bs-8x4+)
                    (progn (setf (aref m 3) (bool-tree c +inter-mode-tree+ (pr-mv-mode p) (* 3 k)))
